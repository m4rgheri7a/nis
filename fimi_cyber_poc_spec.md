...
### 1.2 검증 가설
- **H1(수정)**: 통합 분석(E3)이 콘텐츠-only(E1) 대비 추가 구조 신호를 결합할 수 있는지 검토한다. 실제 OperationalIOC가 0건인 현재 PoC에서는 E3의 단일축 대비 우월성을 최종 성능 주장으로 두지 않고, metadata-level integrated linkage의 구조 검증으로 한정한다.
- **H2**: IOC 노이즈(공유 호스팅 등 저식별력 지표) 비율이 증가할 때 E2 성능은 급락하지만 E3는 완만하게 저하된다 (강건성).

### 1.3 최종 산출물
성능 비교표(E1/E2/E3), 강건성 곡선, 가중치 민감도 히트맵, ablation 표, 상위 사건쌍 증거 경로 시각화(HTML), 자동 생성 리포트(`results/report.md`). 단, 실제 OperationalIOC가 0건이거나 synthetic IOC 의존도가 높은 실행 결과는 논문 Ⅳ장에서 최종 성능 입증이 아니라 구조 검증 및 한계 분석 자료로 인용한다.
